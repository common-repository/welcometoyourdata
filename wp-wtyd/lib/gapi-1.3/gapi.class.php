...
class gapi
{
   ...
   * @param Int $max_results OPTIONAL: Max results returned
   */
  public function requestReportData($report_id, $dimensions, $metrics, $sort_metric="date", $filter=null, $start_date=null, $end_date=null, $start_index=1, $max_results=0)
  {
    ...
    if(is_array($dimensions))
    {
      ...
      foreach($dimensions as $dimesion)
      {
      	if(strpos($dimension, "ga:") !== 0){
	        $dimesion = 'ga:' . $dimesion;
      	}
	    $dimensions_string .= ',' . $dimesion;
      }
      $parameters['dimensions'] = substr($dimensions_string,1);
    }
    else 
    {
      	if(strpos($dimensions, "ga:") !== 0){
    ...
    if($max_results) $parameters['max-results'] = $max_results;
